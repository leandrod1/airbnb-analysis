SELECT host_id, host_name,
    # Because some hosts have more than 1 accommodation, I want to know the number of accommodations per host
    COUNT(host_name) AS number_of_houses,

    # Because the price column has the dollar sign in front of the number, making it a string, we replace it
    # We do the same for the comma ',' for numbers that are in the thousands, then we calculate the revenue in the last 30 days for every host
